...
//Change the name of database to Weathersource
ALTER DATABASE GLOBAL_WEATHER__CLIMATE_DATA_FOR_BI
rename to WEATHERSOURCE;

//To get number of countries in history_day
select distinct country from history_day;

//To get the postal codes starting with '481' and '482'
select count(distinct postal_code) from history_day
...
//Creating a view 
create view DETROIT_ZIPS AS
select distinct postal_code 
from weathersource.standard_tile.history_day
where country = 'US' 
and postal_code like '481%' or postal_code like '482%';

select * from DETROIT_ZIPS;

//filtering the rows in history_day based on postal_code
select hd.postal_code from weathersource.standard_tile.history_day AS hd
join detroit_zips AS dz
where hd.postal_code = dz.postal_code;
...
select min(DATE_VALID_STD), max(DATE_VALID_STD) from weathersource.standard_tile.forecast_day fd
join detroit_zips AS dz
where fd.postal_code = dz.postal_code;

select DATE_VALID_STD, AVG(AVG_CLOUD_COVER_TOT_PCT) from weathersource.standard_tile.forecast_day fd
join detroit_zips dz
